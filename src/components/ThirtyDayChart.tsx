...
import {
  ...
  interface ThirtyDayHistoryTypes {
    date: string
    priceUsd: string
    time: number
}

interface ThirtyDayHistoryProps {
    byDayHistory: ThirtyDayHistoryTypes[]
    coinHistoryId: string
    currentChartDataTime: number
    currentCoinInfoLoading: boolean
}

const ThirtyDayChart = ({ byDayHistory, coinHistoryId, currentChartDataTime, currentCoinInfoLoading }: ThirtyDayHistoryProps) => {
    const [thirtyDayHistoryLabels, setThirtyDayHistoryLabels] = useState<string[]>([])
    const [thirtyPriceLabels, setThirtyPriceLabels] = useState<string[]>([])


    // Slice to get the last 30 objects in the array
    let newByDayHistory = byDayHistory.slice(-32)
   ...
   useEffect(() => {
    ...
    const getNewCoinInfo = () => {
        ...
        setThirtyDayHistoryLabels(newHistoryLabels)

        // setByDayPriceLabels
        setThirtyPriceLabels(newHistoryPrices)
    }
