  ...
  return (
    <div className='transactions'>
        ...
        {transactions.length > 0 ? <div className="transactionsItems container">
            ...
            {transactions.slice(0).reverse().map(item => (
                <div className='transactionsDiv'>
                    <h1>Purchased</h1>
                    <h2>+${item}</h2>
                </div>
            ))}
        </div> : <h2 className='transactionsEmpty'>You haven't made any transactions, add funds in the Wallet tab to view transactions</h2>}
    </div>
  )
}
